...
{
    public sealed record BranchOverflowClearTest : IClassFixture<BranchOverflowClear>
    {
        #region Properties
        private BranchOverflowClear Subject { get; }
        #endregion

        #region Constructors
        public BranchOverflowClearTest(BranchOverflowClear subject)
        {
        ...
        private static Mock<ICpuState> SetupMock(bool isFlag, ushort counter)
        {
            ...
            _ = stateMock
                .Setup(state => state.Flags.IsOverflow)
                .Returns(isFlag);
