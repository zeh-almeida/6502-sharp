...
{
    public sealed record BranchCarryClearTest : IClassFixture<BranchCarryClear>
    {
        #region Properties
        private BranchCarryClear Subject { get; }
        #endregion

        #region Constructors
        public BranchCarryClearTest(BranchCarryClear subject)
        {
        ...
        private static Mock<ICpuState> SetupMock(bool isFlag, ushort counter)
        {
            ...
            _ = stateMock
                .Setup(state => state.Flags.IsCarry)
                .Returns(isFlag);
